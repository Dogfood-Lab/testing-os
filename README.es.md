...
<!-- version:start -->
**v1.12.0** — versión actual. Consulte [CHANGELOG.md](CHANGELOG.md) para ver las novedades.
<!-- version:end -->
...
`testing-os` registra, verifica y aprende de las pruebas reales de su repositorio en un flujo de trabajo nativo de IA. Apúntelo a un repositorio y cada ejecución de prueba se convierte en un registro con confirmación de procedencia en el que puede confiar, y no en un simple resultado autoinformado.

Lo que obtendrá:

- **Registros con confirmación de procedencia.** Cada envío está vinculado a una ejecución de CI real, sin necesidad de claves, a través de la identidad del propio proveedor, antes de ser aceptado. El resultado es un almacén de pruebas inmutable y de solo anexión, no una simple marca verde basada en la confianza.
...
Es el repositorio principal de la organización [Dogfood Lab](https://github.com/dogfood-lab): siete paquetes `@dogfood-lab/*` detrás de una CLI `swarm`.
...
**Las transiciones de estado de los hallazgos son evidencia y de solo anexión.** Los verbos de cierre del plano de control del enjambre (`swarm reopen`, `swarm close`) requieren una razón y una evidencia explícitas, y, para los cierres del operador, un modo de verificación declarado; cada transición escribe una fila `finding_events` inmutable que registra la autoridad que actúa. Ninguna ruta automatizada puede cerrar un hallazgo por inactividad o volver a abrirlo por predicción, y ningún verbo puede reescribir el historial de eventos: una credencial utilizada incorrectamente puede agregar transiciones, pero cada adición se registra.

**Superficie de red.** Por defecto, la única salida es `api.github.com` (solo lectura: confirmación de procedencia + la recuperación de la definición del escenario mencionada anteriormente). Las dos excepciones son opcionales y se describen anteriormente: una entrega de un proveedor de GitLab (`gitlab.com/api`) y una ejecución de anclaje XRPL habilitada por el operador. **No hay telemetría, no hay análisis: esta base de código nunca se comunica con un servidor externo; sin esos dos caminos opcionales, no expone ninguna superficie de red más allá de GitHub.** El flujo de trabajo del receptor se ejecuta con `contents: write`, limitado a este repositorio.
...
| `@dogfood-lab/findings` | JS | Contrato de hallazgo + canalizaciones de derivación/revisión/síntesis/asesoramiento. |
| `@dogfood-lab/ingest` | JS | Conexión de canalización: envío → verificación → almacenamiento → indexación. |
| `@dogfood-lab/report` | JS | Constructor de entregas para repositorios de origen. |
...
Herramientas de prueba independientes que **permanecen independientes** pero se integran a través de API publicadas: [`shipcheck`](https://github.com/mcp-tool-shop-org/shipcheck), [`repo-knowledge`](https://github.com/mcp-tool-shop-org/repo-knowledge), [`ai-eyes-mcp`](https://github.com/mcp-tool-shop-org/ai-eyes-mcp), [`taste-engine`](https://github.com/mcp-tool-shop-org/taste-engine), [`style-dataset-lab`](https://github.com/mcp-tool-shop-org/style-dataset-lab).
