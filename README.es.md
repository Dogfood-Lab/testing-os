...
<!-- version:start -->
**v1.8.0** — versión actual. Consulte [CHANGELOG.md](CHANGELOG.md) para ver qué se ha incluido.
<!-- version:end -->
...
Todos los paquetes `@dogfood-lab/*` se actualizan juntos: un único número en todo el monorepositorio. Seis paquetes se publican en npm bajo `@dogfood-lab` en la versión 1.8.0 de forma sincronizada (`schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`); el séptimo, `@dogfood-lab/portfolio`, permanece interno. La línea de versión cerca de la parte superior de este archivo README se estampa automáticamente desde `package.json` mediante [`scripts/sync-version.mjs`](scripts/sync-version.mjs) en cada ejecución de `npm run build`.
