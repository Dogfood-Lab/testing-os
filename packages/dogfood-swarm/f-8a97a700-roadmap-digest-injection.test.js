 ...
 * — CONTRACT PIN, not yet true in this worktree. Re-authored to the A3.1/
 * A3.2 artifact shapes: `attention` is `{advisory, items[+components]}`,
 * never a flat array (A3.1); the drain half this file exercises is
 * `drain_queue.{entries, overdue_ids}` (A3.2(a), compileAuthoredDrainState's
 * runs-ordinal shape) — the SEPARATE `grandfathered_drain.{frozen_total,
 * drained, outstanding}` section (A3.2(b)) is also populated, structurally
 * valid but empty, so the fixture has BOTH of "the two drain sections", not
 * two halves of one. Neither (b) nor (c) below asserts drain CONTENT
 * specifically — drainCount only bulks the artifact for truncation testing
 * — so the count lands on drain_queue.entries, the T6 "queue" concept the
 * old flat `drain` most resembled. `notes` -> `operator_notes` (A3.1) for
 * the same consistency reason, though inert here too (neither test asserts
 * on notes content).
 */
