 ...
 *      "cadence_runs" } ] }`, and the compiled artifact's `drain` section has
 *      shape `{ entries: [ { id, owner, cadence_runs, runs_since_review,
 *      overdue }, ... ], overdue_ids: [...] }`. If swarm-cp-core lands a
 *      different field/section layout, update the assertions below to match
 *      — the BOUNDARY property under test (>, not >=) is what's load-bearing,
 *      not these exact key names.
 *
...
describe('F-1cd5de59 — drain-queue cadence boundary', () => {
  it('an entry reviewed exactly `cadence` runs ago (5 runs ago, cadence=5) is NOT YET overdue', () => {
    ...
    assert.ok(artifact?.drain?.entries, `expected artifact.drain.entries; got: ${JSON.stringify(artifact)}`);

    const entry = artifact.drain.entries.find(e => e.id === 'F-002109-003');
    ...
    assert.ok(!(artifact.drain.overdue_ids || []).includes('F-002109-003'),
