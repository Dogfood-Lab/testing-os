...
- [x] `[cli]` `--help` output accurate for all commands and flags — `swarm` bin documents its 31 subcommands (init, domains, dispatch, collect, doctor, revalidate, rewind, redrive, reopen, close, roadmap, clean, clean-claims, verify, verify-fixed, verify-recurring, verify-unverified, verify-approved, receipt, advance, adjudicate, status, resume, history, approve, defer, reject, persist, findings, runs, trends) (2026-04-25, last re-affirmed 2026-07-17 — `trends` cross-run analytics verb added in the wave-3 feature pass; `doctor` preflight verb + `collect --all` flag added in the wave-4 CLI-ergonomics feature pass; `clean` worktree-reclaim verb + `dispatch --dry-run` + `--format=json` on advance/receipt/history added in the v1.6.0 feature pass; `defer` + `reject` terminal-disposition verbs added in the feature pass; `adjudicate` cross-family jury verb added in the wave-gating feature pass; `clean-claims` phantom-violation-claims recovery verb added 2026-07-15 after run swarm-1784091637-5127 proved the class live; `reopen` + `close` evidence-bearing closure verbs and the `roadmap` trajectory verb added in the wave-39 feature pass)
