...
- [x] `[cli]` `--help` output accurate for all commands and flags — `swarm` bin documents its 24 subcommands (init, domains, dispatch, collect, doctor, revalidate, rewind, redrive, clean, verify, verify-fixed, verify-recurring, verify-unverified, verify-approved, receipt, advance, status, resume, history, approve, persist, findings, runs, trends) (2026-04-25, last re-affirmed 2026-06-30 — `trends` cross-run analytics verb added in the wave-3 feature pass; `doctor` preflight verb + `collect --all` flag added in the wave-4 CLI-ergonomics feature pass; `clean` worktree-reclaim verb + `dispatch --dry-run` + `--format=json` on advance/receipt/history added in the v1.6.0 feature pass)
...
- [x] `[all]` Version in manifest matches git tag — root + 7 packages all at `1.7.0`, tag `v1.7.0` (2026-06-30)
...
- [x] `[npm]` Six of seven `@dogfood-lab/*` packages are published on npm since v1.2.0 (`schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`); headline install is `npm install -g @dogfood-lab/dogfood-swarm`. The seventh (`@dogfood-lab/portfolio`) remains intentionally workspace-internal. Per-package READMEs carry the canonical logo since v1.2.1. (2026-05-14, last re-affirmed 2026-06-30 at v1.7.0)
- [x] `[npm]` `engines.node` set — root `package.json` has `"engines": {"node": ">=22"}` (tightened from `>=20` to `>=22` in v1.2.2 to match the CI Node 22 + 24 matrix; last re-affirmed 2026-06-30 at v1.7.0)
