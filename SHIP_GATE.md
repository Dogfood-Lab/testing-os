...
- [x] `[cli]` `--help` output accurate for all commands and flags — `swarm` bin documents its 32 subcommands (init, domains, dispatch, collect, doctor, revalidate, rewind, redrive, reopen, close, resolve, roadmap, clean, clean-claims, verify, verify-fixed, verify-recurring, verify-unverified, verify-approved, receipt, advance, adjudicate, status, resume, history, approve, defer, reject, persist, findings, runs, trends) (2026-04-25, last re-affirmed 2026-07-17 — `trends` cross-run analytics verb added in the wave-3 feature pass; `doctor` preflight verb + `collect --all` flag added in the wave-4 CLI-ergonomics feature pass; `clean` worktree-reclaim verb + `dispatch --dry-run` + `--format=json` on advance/receipt/history added in the v1.6.0 feature pass; `defer` + `reject` terminal-disposition verbs added in the feature pass; `adjudicate` cross-family jury verb added in the wave-gating feature pass; `clean-claims` phantom-violation-claims recovery verb added 2026-07-15 after run swarm-1784091637-5127 proved the class live; `reopen` + `close` evidence-bearing closure verbs and the `roadmap` trajectory verb added in the wave-39 feature pass; `resolve` coordinator_resolved closure verb added 2026-07-22 after run swarm-1784601601-bd4a proved the raw-SQL closure class live)
...
- [x] `[all]` Version in manifest matches git tag — root + 7 packages all at `1.10.0`, tag `v1.10.0` (2026-07-17)
...
- [x] `[npm]` Six of seven `@dogfood-lab/*` packages are published on npm since v1.2.0 (`schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`); headline install is `npm install -g @dogfood-lab/dogfood-swarm`. The seventh (`@dogfood-lab/portfolio`) remains intentionally workspace-internal. Per-package READMEs carry the canonical logo since v1.2.1. (2026-05-14, last re-affirmed 2026-07-17 at v1.10.0)
- [x] `[npm]` `engines.node` set — root `package.json` has `"engines": {"node": ">=22"}` (tightened from `>=20` to `>=22` in v1.2.2 to match the CI Node 22 + 24 matrix; last re-affirmed 2026-07-17 at v1.10.0)
