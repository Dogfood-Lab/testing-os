...
- [x] `[cli]` `--help` output accurate for all commands and flags — `swarm` bin documents its 23 subcommands (init, domains, dispatch, collect, doctor, revalidate, rewind, redrive, verify, verify-fixed, verify-recurring, verify-unverified, verify-approved, receipt, advance, status, resume, history, approve, persist, findings, runs, trends) (2026-04-25, last re-affirmed 2026-06-13 — `trends` cross-run analytics verb added in the wave-3 feature pass; `doctor` preflight verb + `collect --all` flag added in the wave-4 CLI-ergonomics feature pass)
