...
**v1.2.0** — primeira publicação npm do monorepório `@dogfood-lab/*`. Seis pacotes agora estão disponíveis publicamente sob o escopo `@dogfood-lab`: `schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm` (o CLI de "enxame" principal). Novidades nesta versão: máquina de estados de nível de onda + contrato de recuperação de "Três Rs" (`swarm revalidate`, `swarm rewind`, `swarm redrive`) + verbo de histórico de auditoria `swarm history` + verificação de saúde de Estágio A–D em 0 CRÍTICO / 0 ALTO. **1105/1105 testes.** Total acumulado ao longo da vida do repositório (desde a versão v1.0.0, corte em 25 de abril de 2026): tudo o acima listado, mais o dogfood swarm da Fase 7 (~31 ondas, ~115 correções verificadas, 14 classes de cobertura de auditoria). Catálogo de enxame autorizado: [`docs/swarm-evidence-2026-04-27.md`](docs/swarm-evidence-2026-04-27.md).
