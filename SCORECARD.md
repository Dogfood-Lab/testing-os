...
**Date:** 2026-06-01 (re-affirmed at v1.3.0; first scored 2026-04-25 at v1.0.0; re-affirmed 2026-05-14 at v1.2.0; re-affirmed 2026-05-31 at v1.2.3)
...
Sourced primarily from [`SHIP_GATE.md`](SHIP_GATE.md): at v1.3.0 (re-affirmed 2026-06-01), every applicable hard-gate A–D row carries either an `[x]` evidence stamp or a `SKIP:` with explicit justification — `shipcheck audit` at the v1.3.0 release tree exits 0 (21 checked / 16 SKIP-with-justification / 0 unchecked). Soft gate E is fully met. The "100% pass on hard gates A–D" headline phrasing reflects the audit-tool verdict, not a hand-curated estimate; the per-row evidence dates below are the auditable substrate.
...
Honest deltas: every SKIP that remains in SHIP_GATE.md (the MCP / desktop / VSCode items that don't apply, plus the real follow-ups above) is the reason individual category scores stay below 10. The repo is shippable at v1.3.0 by every applicable contract — six of seven `@dogfood-lab/*` packages are live on npm since v1.2.0; the gap is between "shippable" and "perfect."
