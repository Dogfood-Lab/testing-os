...
| **No identifier at all — never extractable, never gated** | Distinct from the URL/PDF-shaped Unparsed bucket (wave-22 correction, F-282d0d4a): three citations carried neither a DOI/arXiv id nor any URL, so no extractor of any tuning could have surfaced them — finding 1's De Lucia et al. (the first citation supporting C1), finding 17's DeMillo/Lipton/Sayward 1978, and finding 26's Chen/Cheung/Yiu 1998. All three now carry identifiers at their citation sites (DOI 10.1145/1368088.1368216 · DOI 10.1109/C-M.1978.218136 · arXiv:2002.12543), **existence-and-attribution verified out-of-band against the Crossref / arXiv resolvers directly (2026-07-16)**. Their groundedness remains **unverified-by-this-oracle** — the same boundary as the 14 oracle-limited items above — and none of the three has ever passed the Step-4 runner itself. |
...
17. **Mutation testing rests on 45+ year-old foundations and a mature tool ecosystem, not an experimental bet.** DeMillo, Lipton & Sayward 1978 (*Hints on Test Data Selection: Help for the Practicing Programmer*, Computer 11(4):34–41, DOI 10.1109/C-M.1978.218136 — identifier added wave 22, F-282d0d4a, Crossref-verified); Jia & Harman 2011 (*An Analysis and Survey of the Development of Mutation Testing*, IEEE TSE 37(5):649–678, DOI 10.1109/TSE.2010.62). **Implication:** adopting mutation as the oracle applies established theory.
...
35. **Full-AST parsing is cheap in absolute terms — a large real-world TypeScript file parses in 26.4ms (oxc) / 91.0ms (swc) / 139.3ms (Biome), single-threaded.** oxc-project, *bench-javascript-parser-written-in-rust* (https://github.com/oxc-project/bench-javascript-parser-written-in-rust). **Implication:** at the few-hundred files scanned once per `npm run verify` (the live gate prints the exact count each run — no hardcoded figure here; an earlier "553" was already stale by ~40, F-53eb836d), parse cost is not a credible objection to dropping the heuristics.
...
- **Tier 1 (blocking):** the declared-link check (C1–C3). It is exact — a tag either parses, validates and attaches, or it does not. Zero inference, so zero false grants *and* zero false orphans. This blocks `verify`/CI/`release`. **Scope of the exactness claim (wave 22, F-8d789764): it describes Tier 1's verdict on *declared* ids only.** The pin corpus at cutover is majority-grandfathered — exempted-and-disclosed, never credited — per the migration strategy below; C5 makes no claim about that bucket.
...
- **Ids not yet drained live in a frozen grandfather manifest — exemption-with-disclosure, never verified credit.** *(Findings 20, 21; section added wave 22, F-8d789764 — the shipped gate's dominant bucket was previously absent from this contract.)* The cutover could not stop the world while every legacy pin was hand-vetted, so the ids the retired heuristic credited at the freeze commit (`132dc18`) that lacked a vetted declared tag were snapshotted **once** into `scripts/grandfathered-pins.json`: closed, membership-only, never re-evaluated by any live heuristic. The gate names the bucket in its own output as UNVERIFIED migration debt (per C6) — a grandfathered id is never presented as proven, and a grandfathered id is not a false grant (nothing is claimed for it beyond its enumeration). Entries carry provenance and a revalidation owner (C8's discipline, against finding 21's ~1-in-2 suppression-rot base rate), and the set only drains: `scripts/suggest-pins.mjs` proposes a declared tag, the owning domain vets and lands it (finding 5's candidate-then-human shape, above). **Integrity requirement: membership is content-addressed** (canonical serialization + SHA-256, the same discipline the ingest hash-chain uses — the gate computes `grandfatherManifestFingerprint()` over the whole manifest incl. its header and checks it against `EXPECTED_GRANDFATHER_MANIFEST_HASH` pinned in the gate's own source) so an id cannot be swapped under a constant headcount by a later, isolated edit — a lawful drain is a deliberate, reviewed hash update (wave 22, F-dbc8b0d1). This is **tamper-evident, not tamper-proof** (corrected wave 26, F-359bb9a3): an actor who edits the manifest AND recomputes/re-pins `EXPECTED_GRANDFATHER_MANIFEST_HASH` in the *same* commit passes clean — the identical boundary `packages/ingest/lib/integrity.js` discloses for the record hash-chain, and now disclosed in the gate's own `DISCLOSED_GAPS` (F-7428a68f). The hash defeats a drive-by manifest edit; it does not defeat a hostile committer with write access.
...
4. **The corpus can still be outrun.** No corpus proves non-leakage — C9's generator + relations are mitigations, not proofs *(findings 24, 28; an earlier revision cited finding 29 here, which was withdrawn at Step 4 — caught by the wave-22 sibling sweep for claims resting on retracted support, F-282d0d4a)*.
5. **The grandfathered majority.** At the cutover, the frozen grandfather manifest holds the large majority of source-pinned ids — declared-and-verified tags are the minority the migration has drained so far. "The gate is green" therefore means *declared ids verified; frozen debt named and counted* — never *all pins verified*. The exact live split (declared / grandfathered / allowlisted / orphan) is printed by the gate's own output every run and is the authority; a hardcoded ratio here would drift as the manifest drains, so none is quoted (an earlier revision cited "≈84%", already stale against the live counts by the next wave — the same volatile-counter trap HANDOFF.md's own history documents). The bucket is disclosed in the gate's output (C6), integrity-guarded by content-addressing (F-dbc8b0d1), and drains per the migration strategy above. *(Added wave 22, F-8d789764; the hardcoded ratio was removed in favor of the live gate wave 24, F-b9b5c3e5.)*
