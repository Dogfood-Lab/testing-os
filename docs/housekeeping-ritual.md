...
**0. Revisit the operator notes — every swarm, no exceptions.** Read `dogfood/roadmap-notes.json` back at session start. The notes come in two classes, differentiated in their own text: **STANDING ORDERs** (permanent operating rules — re-affirm, never silently drop) and **TRAJECTORY** notes (finite direction that moves the repo forward — check each for completion; a completed trajectory note gets retired and its replacement proposed to the Director in plain English, per the plain-English-first rule). The Director's direction: the point of the notes is to move the repo forward, so a session that leaves every trajectory note untouched should say why. Done when: every note re-read, completions retired, the compile re-run if the set changed.
