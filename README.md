...
<!-- version:start -->
**v1.2.1** — 7 packages (`@dogfood-lab/*`), workspace-wide test suite, ingest receiver live, handbook deployed.
<!-- version:end -->
...
**v1.2.1** — patch release adding the testing-os logo to every per-package README so it renders on each npm page (v1.2.0 same day shipped three packages without the logo — v1.2.1 lockstep bump corrects all 6). All Phase 5 features carry over from v1.2.0: wave-level state machine + Three R's recovery contract (`swarm revalidate`, `swarm rewind`, `swarm redrive`) + `swarm history` audit-trail verb + Stage A–D health pass at 0 CRIT / 0 HIGH. Six packages publish under `@dogfood-lab`: `schemas`, `verify`, `report`, `ingest`, `findings`, `dogfood-swarm`. **1105/1105 tests.** Cumulative across the repo lifetime (since v1.0.0 cut 2026-04-25): the Phase 7 dogfood swarm (~31 waves, ~115 verified-holding fixes, 14 audit-coverage classes) and now the v1.2.x first-npm-publish arc. Authoritative swarm catalog: [`docs/swarm-evidence-2026-04-27.md`](docs/swarm-evidence-2026-04-27.md).
