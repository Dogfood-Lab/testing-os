...
- **`closure_kind`** — `declared` (owning-domain `confirmed[]`), `operator` (`swarm close`), or `absence` (a full-coverage audit's by-absence closure; recorded by the classifier only, reachable from no verb).
