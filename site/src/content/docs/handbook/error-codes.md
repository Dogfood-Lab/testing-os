...
- **Not every junk value reaches this code.** `Number('')` is **`0`**, not `NaN` — finite — so an **empty** `schema_version` does *not* throw here; it reads as version 0 and takes the bootstrap path. The same is true of anything else `Number()` coerces to a number (`" "`, `"0x10"`). This is the `Number('') === 0` trap that `F-b5fd9887` fixed one module over in `cross-run-analytics.js`, and an earlier revision of this very bullet listed `""` as a trigger — wrong, and caught by an audit that ran the code instead of reading the prose.
