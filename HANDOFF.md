...
>
> **Scope split:** wave 18 = Tier 1 (declared-link gate on a real AST, C1–C3/C6/C7/C8) + the **full C9 assurance rig**. Wave 19 = Tier 2 (diff-scoped mutation proof, StrykerJS) + the confirming audit. The old heuristic is **retired into the role the literature says it was always valid for**: `scripts/suggest-pins.mjs`, a candidate-link generator that grants nothing.
>
> **WAVE 18 IS LANDED — committed `132dc18`, serial verify green, `swarm verify` receipt #20 (3,311 tests, 0 fails).** The gate now reads a schema-validated `@pins` tag from a real AST (`@babel/parser`) and reports, in its own output: **7 declared (AST-verified) · 256 grandfathered-and-named as UNVERIFIED migration debt · 0 orphans · 0 parse errors** across 285 pins / 569 files. The old gate claimed the invariant held while a comment could forge a pin; **the new gate's claim shrank to the truth.** C9 shipped whole (second independently-coded matcher diffed over corpus + live tree; metamorphic relations run generically; syntax-space generator; 7 matcher-mutants each proven killed; append-only corpus carrying all 7 historical leaks as generalized relations) — and it caught three real defects in its own submission before they shipped.
>
> **Open debt carried into wave 19 (disclosed, not hidden):**
...
> 2. **Tier 2 not built** — a vacuous-but-correctly-declared pin still passes. This is the pseudo-tested-method class (~10% base rate); no parser can see it. Wave 19.
...
>
> **To resume:** read the dispatch (`docs/pin-matcher-rewrite.dispatch.md`) — it is the contract — then `swarm status swarm-1784091637-5127` for live state. Wave-18 agent evidence lands in `swarms/swarm-1784091637-5127/wave-18/*/output.json`. The loop from there: collect → ONE **unpiped** `npm run verify` → commit → `swarm verify` (records the receipt the advance gate needs) → advance → wave 19.
>
