...
> ## ⏸ Stage A health pass — PAUSED at wave 17 for a Director decision (2026-07-16)
>
> Run `swarm-1784091637-5127`, branch **`swarm/health-amend-a-1784091637`** (not merged to `main`). **For live wave / phase / branch-HEAD / commits-ahead, run `swarm status swarm-1784091637-5127`** (or `git log --oneline main..HEAD`). This block records durable facts + completed history, never a volatile "current wave is N, HEAD is X" snapshot — that shape went stale within one commit *three* times this run (F-c467def9, F-d39dd777, and F-f164bb17 — the last one in the very block this replaces, which mislabeled wave 16 "in progress" while being written by wave 16's own commit). Resist re-adding volatile counters here.
>
> **Where it stands:** wave 16 (the lex-then-credit pin-matcher rewrite) is committed at `a728d59`, deterministic floor green (`swarm verify` receipt #19). **The wave-17 confirming audit ran and Stage A did NOT exit clean** — **0 CRITICAL, 3 HIGH, 7 MED, 2 LOW** (12 new findings; all 12 approved). **The Director chose Option C (comprehensive) and a study-swarm ran first — its research-grounded design is committed as [`docs/pin-matcher-rewrite.dispatch.md`](docs/pin-matcher-rewrite.dispatch.md) (`3bad3d9`), which is now the CONTRACT for the rewrite.** **For live wave/phase, run `swarm status swarm-1784091637-5127`. For what is PLANNED next, read the dispatch — it owns the plan; this block does not.**
>
...
> | w13 | 3 | w14 | `079103e` |
> | w15 | 2 | w16 | `a728d59` |
> | **w17** | **3 — did not exit** | — | *decision pending* |
>
