...
> | **w21** | **6 — did not exit** | — | *26 approved, queued in the control plane* |
>
...
>
> **THE NEXT SESSION STARTS HERE.** All **26 wave-21 findings are approved and queued in the control plane** — they route to the next amend automatically. **Do not reconstruct them from this file or from any `output.json`; read the DB** (`swarm status`, or the generated brief). The 6 HIGH, by canonical id: `F-b9de9cb6` + `F-d4e09870` (verify-classifier / verify-fixed: an asymmetric ±1 window — 10 lines down, 1 up — lets an untouched bug be reported `verified`; the module built to catch "agent claimed fixed, bug never moved" does that very thing, and its docblock claims the symmetry it lacks. **Scoping: this run's verdicts came from audit fixed-by-absence, not this classifier — the record is not corrupted; this is a latent product defect**); `F-dbc8b0d1` (the frozen manifest's only integrity guard is `length === 256` — a headcount; swap a drained id for a fresh one and it holds. Fix: content-address it, per the `dispatch.lock.json` discipline); `F-cc198701` (`partial` is unchecked in the verdict/step-evidence rule — 7 failing steps accepted with zero rejections: a free verdict-inflation path); `F-51780da9` (the TOO_NEW routing fix silently flipped `retryable`, so a run_id can **never** be accepted again even after the upgrade that would fix it — contradicting that fix's own test comment); `F-f1dae277` (ownership-violation **file paths** sit at the same zero-privilege boundary as the fixed `probe.reason` and render unescaped at three sites, incl. the checked-in GitHub-rendered receipt — and one instance passes the meta-test today *only* because the file escapes an unrelated field elsewhere).
>
> **Two findings worth reading even if you fix nothing else:** the Unicode **Tag block (U+E0000–E007F) is unescaped** — the ASCII-smuggling primitive Claude models preferentially decode, and this tool's render surfaces are read by Claude-driven coordinators (i.e. a prompt-injection path into *us*, from any repo we audit). And the same escape range is **over-broad**, mangling legitimate NFD Vietnamese/French/Greek and shattering ZWJ emoji — wrong in both directions at once.
>
