...
>
> Run `swarm-1784091637-5127`, branch **`swarm/health-amend-a-1784091637`** (not merged to `main`). **For live wave / phase / branch-HEAD / commits-ahead, run `swarm status swarm-1784091637-5127`** (or `git log --oneline main..HEAD`). This block records durable facts + completed history, never a volatile "current wave is N, HEAD is X" snapshot — that shape went stale within one commit *three* times this run (F-c467def9, F-d39dd777, and F-6b19cf3e — the last one in the very block this replaces, which mislabeled wave 16 "in progress" while being written by wave 16's own commit). Resist re-adding volatile counters here.
>
> **Where it stands:** wave 16 (the lex-then-credit pin-matcher rewrite) is committed at `a728d59`, deterministic floor green (`swarm verify` receipt #19; `npm run verify` 0 fails). **The wave-17 confirming audit ran and Stage A did NOT exit clean** — it found **0 CRITICAL, 3 HIGH, 7 MED, 2 LOW** (12 new findings, recorded `new` in the control plane). Wave 17 is `collected` at phase `health-audit-a`. **The next move is a Director decision (see THE DECISION below) — nothing is dispatched, nothing is merged.**
>
...
> 1. **`scripts/check-finding-regression-pins.mjs` — pin-matcher false-grant (the 7th leak).** The wave-16 rewrite masks non-code for every credit rule EXCEPT `isLeadingCommentPin` (line 669), which runs on raw text and never verifies comment-ness: `STRUCTURAL_COMMENT_DECORATION` fails to match an undecorated line, the following `.replace()` no-ops, and `line.startsWith(id)` credits any column-0 line — so a string/template continuation line beginning with an F-id falsely greens the Class #14 gate. Proven live via `--root` fixtures; **not present in the real 553-file corpus** (protects the actual repo today). Paired MED (`F-002`): `matchTestCallStart` truncates a template-literal title at the first `${`, a false-orphan.
> 2. **`packages/dogfood-swarm/commands/history.js` (`F-7c3e91a4`) + `commands/status.js` (`F-2f8b56d0`) — unescaped `--reason`.** A newline in an override reason forges a fabricated transition/breadcrumb row; raw ANSI (`\x1b[…`) can erase an already-printed genuine line; a lone `"` reopens the F-fa23cc37 quote-forgery class. `--format=json` stays truthful. `status.js` is the DEFAULT view every operator runs. Requires control-plane write access; corrupts only the text view.
>
> **The root pattern (the load-bearing lesson):** every wave-16 fix patched *the instance in front of it*, not *the class*. The pin-matcher rewrite masked all rules but one. The wave-16 escaping fix (`F-11f0e453`) touched ONE call site (`escapeReasonForDisplay` in cli.js) — but the SAME unescaped-reason rendering lives in SEVEN sites (history.js, status.js, rewind.js, redrive.js, revalidate.js, clean-claims.js, formatOverrideGroups); it was never extracted to a shared `lib/` helper. The verdict-agreement fix aligned two fields and broke a third (core `F-6b2f8e14`, MED — aborted `overall_posture:'critical'` next to zero counts). **The wave-17 ci-tooling auditor reports this as the 7th consecutive audit to find a live pin-matcher evasion** — the "if it leaks a 7th time, the approach was wrong, not the patch" rule this note has carried since wave 14 has now fired.
>
> **THE DECISION (Director paused here to choose fresh — pick one):**
> - **(A) One class-fix wave, then exit.** Wave 18 as CLASS fixes, not instance patches: extract ONE shared reason-escape helper and route all 7 render sites through it; run the pin rule on the comment-masked view + finish the template-title lexer; fix the wave-16-introduced posture MED (`F-6b2f8e14`). Wave 19 confirms. Breaks the instance-patch pattern that caused this; costs 2 waves.
> - **(B) Accept + document, exit now.** All 3 HIGH are proven-not-live (the gate protects the real repo today; the escaping needs a self-inflicted malicious `--reason` and only corrupts the text view). Re-triage the 2 escaping HIGHs to elevated-MED vs the `F-11f0e453` precedent, log the pin-matcher false-grant as a documented not-live residual with a regression pin, merge the branch.
> - **(C) Rewrite the pin-matcher on a real parser.** Replace the hand-rolled lexer + per-rule heuristics with the TypeScript scanner (already a repo dep via `tsc`) so "comment / string / title / code" is answered by a real tokenizer — no rule can forget masking. Definitive; bigger; its own wave. Escaping + docs fixes ride along.
>
> **To resume:** read this block, then the 12-finding ledger at `swarms/swarm-1784091637-5127/wave-17/*/output.json`. Pick A/B/C. For an amend (A or C): triage the 12 findings, then `swarm advance <run>` (health-audit-a → health-amend-a) → `swarm dispatch <run> health-amend-a --isolate --skip-verify` → 6 agents → collect → ONE unpiped `npm run verify` → commit → `swarm verify` → advance → confirming audit. For exit (B): re-triage the escaping HIGHs, document the pin-matcher residual, merge to `main`. The wave-16 worktrees (`.swarm/worktrees/w16-*`) are merged + redundant and can be `git worktree remove`d.
>
